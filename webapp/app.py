...
app.config['MAIL_DEFAULT_SENDER'] = os.environ.get('MAIL_DEFAULT_SENDER', 'noreply@cyberscan.local')
...
def api_key_required(fn):
    ...
    def wrapper(*args, **kwargs):
        ...
        if used >= 100:
            return jsonify({'error': 'Daily API limit reached (100/day)'}), 429
...
def login():
    ...
    return render_template('login.html', locked_until=locked_until, next=request.args.get('next'))


def _login_post():
    identifier = (request.form.get('username') or '').strip()
    password = request.form.get('password') or ''
    remember = request.form.get('remember') == '1' or request.form.get('remember') == 'on'
    user_row = User.by_username_or_email(identifier)
    db = get_db()
    now = datetime.datetime.utcnow()

    def fail_response(msg, locked_iso=None, failures=0):
        next_q = (request.form.get('next') or request.args.get('next') or '').strip() or None
        payload = {'ok': False, 'message': msg, 'failures': failures, 'locked_until': locked_iso}
        if request.is_json or request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            return jsonify(payload), 401 if not locked_iso else 423
        flash(msg, 'error')
        return render_template('login.html', locked_until=locked_iso, next=next_q), 401 if not locked_iso else 423
    ...
    if not identifier or not password:
        logger.warning('Failed login: %s from %s', identifier or '(empty)', _client_ip())
    ...
    row = db.execute('SELECT last_login, last_login_ip FROM users WHERE id = ?', (user_row.id,)).fetchone()
...
def check_username():
    ...
    db = get_db()
    taken = db.execute('SELECT 1 FROM users WHERE username = ?', (u,)).fetchone() is not None
...
def check_email():
    ...
    if not e or not re.match(r'^[^@\s]+@[^@\s]+\.[^@\s]+$', e):
        return jsonify({'available': False, 'invalid': True})
    db = get_db()
    taken = db.execute('SELECT 1 FROM users WHERE email = ?', (e,)).fetchone() is not None
    return jsonify({'available': not taken})
    ...
    try:
        if '@' in query:
            ...
            if r.status_code == 404:
                ...
                return jsonify({'error': 'HIBP API error', 'status': r.status_code}), 502
        else:
            out['mode'] = 'domain'
            enc = urllib.parse.quote(query)
            r = requests.get(
    ...
    for sch in rows:
        ...
        if is_private_ip(sch['target']):
            ...
            c.execute(
                ...
                (now.isoformat() + 'Z', _next_run_iso(sch['frequency'], now), sch['id']),
            )
            db.commit()
            continue
        ...
        if sch['notify_email'] and app.config.get('MAIL_SERVER'):
            user = c.execute('SELECT email, username FROM users WHERE id = ?', (sch['user_id'],)).fetchone()
...
def revoke_api_key(kid):
    db = get_db()
    db.execute('DELETE FROM api_keys WHERE id = ? AND user_id = ?', (kid, current_user.id))
    db.commit()
    return jsonify({'ok': True})
...
@api_key_required
def api_ssl():
    host = request.args.get('target', '').strip().replace('https://', '').replace('http://', '').split('/')[0]
...
def scan():
    ...
    try:
        ...
        return jsonify({'error': str(e)}), 500
    out = [{'port': p, 'service': s, 'banner': b} for p, s, b in results]
    return jsonify({'target': target, 'results': out, 'ai_analysis': analyze_port_scan(out)})
...
def headers_analyze():
    ...
    logger.info('Scan: headers_analyze on %s by %s', hdr_host or url, current_user.username)
    try:
        r = requests.get(
            url,
            timeout=14,
    ...
    try:
        ...
        for v in body.get('vulnerabilities') or []:
            ...
            for key in ('cvssMetricV31', 'cvssMetricV30', 'cvssMetricV2'):
                ...
                sev = (cd.get('baseSeverity') or arr[0].get('baseSeverity') or '').upper()
                break
